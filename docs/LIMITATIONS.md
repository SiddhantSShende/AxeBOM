...
| CBOM | **Source code is read for Java, Python and JavaScript/TypeScript only, and without a build.** `cbomkit-action` (sonar-cryptography's rules) reads Java and Python; `cdxgen-cbom` reads JS/TS call sites and misses `crypto.createCipheriv` and WebCrypto. A call whose algorithm arrives through a variable or a wrapper is invisible without a build — on jjwt no RSA or EC call was found — and where code states no key size the engine reports its default (AES-128). **Go, C#, C/C++ and every other language have no source engine**: Go's scanner cannot run under the noexec sandbox and fails silently, so it is disabled rather than trusted, and each appears in Engine Coverage as a language with no engine. JAR/WAR bytecode is not analysed. `cbomkit-theia` reads key, certificate and crypto-config files, never code; scanning an exported image tarball takes minutes even for a small image. `cdxgen-cbom` is not started on a tree with no JS/TS, and on a tree with JS/TS **and** a Java build it can hang in its atom analysis — reported as a five-minute `timeout`, never as an empty result. theia's secret detection (gitleaks rules) reports possible credentials, which are listed as `CBOM_SECRET_IN_SOURCE` findings, not counted as cryptographic keys; its rules also match examples and placeholders in documentation. |

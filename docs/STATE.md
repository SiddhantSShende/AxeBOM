...
**Current phase:** SBOM completion pass (revisiting Phases 7–9) — 🟢 **The SBOM module's single biggest gap is closed AND live-proven end to end against two real, uncontrolled public repos, INCLUDING report rendering (SPDX/CycloneDX/PDF/XLSX/JSON) — a real scan now normalizes for real with no manual trigger, and reports render for real off the normalized data, driven through the actual product API (an API key + `POST /v1/scans`), not just a throwaway Go program. A full three-way adversarial audit (Go/Python/migrations, run live) plus a live scan through the real API surface together found and fixed one high-severity production bug (a scan could silently, permanently never normalize after one transient NATS/DB blip), one data-fabrication bug (a hardcoded CVSS version), one crash (any API-key/service-authenticated scan trigger 500'd), and one false-negative compliance bug (a fully-covered ecosystem could be reported as an Engine Coverage gap). Both golden fixtures Gap C deferred are now built.** `workers/sbom/normalize_consumer.py` (new) is what does the normalizing; see the 2026-08-26 (a) through (e) and 2026-08-27 (f) session entries.
**Next action:** 🟢 Everything explicitly scoped by this session's plan is done, audited, and now confirmed against a real API-driven scan (not just direct-DB throwaway programs). 🟡 One real, deliberately-unfixed gap remains, documented on `normalize.bom_documents.alias_snapshot_id` in `docs/01-DATA-MODEL.md` and in (e): no FK exists from that column to `normalize.alias_snapshot.id` because `qbom.go`/`hbom.go` write it as a bare fabricated uuid with no backing row — fixing it correctly means deciding what QBOM/HBOM should reference, which is out of SBOM-only scope. 🟡 `NVD_API_KEY` is NOT actually set (`.env`'s value is genuinely empty, only whitespace before its `# SECRET` comment) — grype/osv/trivy databases ARE provisioned (~3.5 GB, confirmed via `task osint:dbstatus`); `dependency-check` correctly reports `unavailable` without a real key from https://nvd.nist.gov/developers/request-an-api-key. Provisioning must run **inside a container as root** (`docker compose run --rm --no-deps -T sbom-worker python -m workers.sbom.dbsync ...`), not on the host as a non-root user (`osv`'s direct-bind-mount write lands root-owned). Disk space, flagged 🔴 as of (b), is now healthy again — see (c) (49.5 GB reclaimed). 🟡 **Containers must be rebuilt+redeployed after any code change** — (f) found every relevant container was still running a stale pre-session image; there is no auto-rebuild/watch in this dev stack, so a session that edits code and never runs `docker compose build <service> && docker compose up -d <service>` leaves the running app unchanged regardless of what's on disk. Nothing has been committed to git this session — everything above is still uncommitted working-tree changes.

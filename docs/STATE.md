...
**No download path.** `ScanProgress.tsx` showed live progress and the per-engine table but nothing about the reports `/generate` had queued for that scan — a user had to know to navigate to the separate reports list and find the right row. Added a Reports section (`GET /v1/reports?scan_id=…`, the existing `useReports(scanId)`) with a real download link per report the moment it reaches `ready`, reusing `ReportViewer.tsx`'s own `<a href=".../download" download>` pattern rather than inventing a second one. `useReports` itself had no `refetchInterval` at all — unlike its singular sibling `useReport` — so a report still `queued`/`rendering` would never flip to `ready` on screen without a manual reload; generalized `useReport`'s own poll-while-pending/stop-once-terminal schedule to the list. `levelLabel` moved out of `ReportViewer.tsx` (private, about to be duplicated) into `lib/reports.ts` so both pages share one definition.

**Two honesty bugs, both reported directly by the user looking at a real scan:**
- *"if the engine is running then why is it showing 100%"* — `deriveActivity` (from (b)) seeded a `phase: 'running'` entry — present tense, pulsing "live" dot — from an engine's `started_at` even when that same engine had also finished. For a scan that finished 36 minutes earlier this read as "happening right now" beside a bar already at 100%. Fixed: a "started" entry is only synthesized when the engine has *not* also finished; a genuinely still-running engine keeps it, correctly.
- *"it should show 100% when the entire report and entire scanning is done"* — the bar hit 100% the moment every engine went terminal, before any report existed to download. `services/report`'s worker resolves the normalized BOM at *render* time, not scan time ((a)'s own `loadNormalizedBOM` fix), so a report can finish seconds after its scan — "100%, completed" could show with nothing yet downloadable. `displayPercent` now holds the bar at 99% once the scan is terminal but any of its reports are still `queued`/`rendering`, reaching a true 100% only once every report this scan produced has also finished. Deliberately not a blended client-side percentage — `docs/02-CONTRACTS.md §5` still owns weighted scan progress server-side; this is only a "not quite yet" gate on the display, with a note explaining why. The reports query moved up to `ScanProgressRoute` so the bar and the reports list observe the same poll rather than risking visible disagreement between two separate ones.

**Verification:** `npm run build`/`lint`/`test` (96 tests) clean. Rebuilt and redeployed `frontend`. Confirmed against real data: `RNR Consulting` has two scans with `ready` CycloneDX reports (`01a0542b-56fa-…`, `01a0543d-2092-…`) that now show a working download button; the scan the user was actually looking at when they asked "where are the reports" turned out to be one of this session's own diagnostic API-key-triggered scans ((b)'s live-verification scans), never run through Generate and so never carrying a report — not a product gap, confirmed by querying `report.reports` directly rather than assumed. Commit `601c9e6`.

### 2026-08-30 (b) — live scan progress: a real backend event stream nobody had ever wired up, an animated UI on top of it, and a frame-parsing bug that had silenced the whole page in production

**Goal:** the user asked for an animated progress range and a real-time "what is it doing now" feed on the scan progress page, showing only the last three items.

**The foundational bug, found before any UI work could matter:** `frontend/src/lib/ws.ts`'s `parseFrame` read flat top-level fields (`scan_id`, `percent`, `engines[].engine_id`) that `services/scan-orchestrator/internal/handler/handler.go`'s `wsMessage` has never actually sent — the real wire shape nests everything under `"snapshot"` (a `scanDTO`, keyed `id`/`progress_pct`/`engine_runs[]`, each run keyed `engine` not `engine_id`) or `"event"` (an `events.ScanEventV1`). Every real frame the server had ever sent was silently dropped — `onProgress` had never fired once in production, independent of anything else (including this session's earlier WS-origin fix) being fixed. `ws.test.ts`'s own fixtures asserted the same wrong shape, which is exactly how this shipped unnoticed. Rewrote `parseFrame` against the real Go structs; split the snapshot (authoritative structured state — percent, per-engine status/ecosystems/duration) from an event (advisory human-readable activity) as two genuinely different things rather than pretending one function handles both, matching what the file's own design-intent comment already said but the implementation never did.

**The real event stream — built, never wired:** `Orchestrator.PublishEvent` existed fully built (seq/ts/sanitize/validate, `docs/02-CONTRACTS.md §10`'s own envelope) since Phase 6. Grepping the entire codebase for `.PublishEvent(` and for `events.ScanEventV1{` construction outside test/schema code found **zero** real call sites — `scan.event.<id>` had a genuine WS consumer (`streamEvents`) and nothing had ever published to it. Wired it in for real:
- `services/scan-orchestrator/internal/orchestr/orchestrator.go`: `FanOut` publishes "running `<engine>`" the moment a job actually dispatches; `HandleResult` publishes "`<engine>` `<status>`" once its result lands — both carrying the same weighted `Progress()` percentage the snapshot itself uses.
- `services/webrecon/internal/work/work.go`: publishes "discovered N host(s)" after `resolveHosts`, then a live "fingerprinting `<host>` (i of N)" event for each host as `fingerprintAll` works through them — the one place in the whole product where a scan's progress genuinely has more than two states (dispatched, done) to show while it runs.
- `libs/go-shared/events/publish.go` (new): extracted the shared construction logic (both producers now call the same `PublishScanEvent`) rather than let a second implementation drift from the first the way this whole feature had drifted from ever being called at all. `orchestr.Orchestrator.PublishEvent` is now a two-line wrapper preserving its own injectable-clock test behavior.

**The UI** (`frontend/src/routes/scans/ScanProgress.tsx`): the overall bar now spring-eases toward each real percentage via `motion` (already a house dependency — `motion/react`, the same `m`/`AnimatePresence`/`LazyMotion` pattern `ProjectList.tsx`/`ReportViewer.tsx` already use), with a live shimmer gated on the scan not yet being terminal. A new "Right now" panel shows the last three real activity items, animated in and out. **For a scan already finished by the time the page connects — the common case for a fast webrecon scan, which will produce zero live events since the server sends the snapshot and closes immediately** — the feed seeds itself from that same snapshot's own `engine_runs` history (`started_at`/`finished_at`/`status`/`message`, all already real, recorded values) rather than sitting permanently empty. Nothing in either path is invented.

**A second real bug, surfaced only by the new per-host visibility, found live:** `services/webrecon/internal/discover`'s `parseHosts` accepted *any* non-blank stdout line from subfinder as a discovered hostname. `discover.Policy()`'s read-only rootfs blocks subfinder's first-run config write; the resulting error — `"open /.config/subfinder/config.yaml: no such file or directory"` — reaches stdout despite `-silent`, indistinguishable by position from a real result. Confirmed live: a scan for RNR Consulting genuinely fingerprinted that literal string as if it were a subdomain, before anyone could see individual host names to notice. Added hostname validation (`looksLikeHostname`) and test coverage for a package that had zero tests before this.

**Verification actually performed:** captured the real WebSocket byte stream from real triggered scans (not just unit tests) — before and after each fix — confirming snapshot → discovery → per-host fingerprint → completion events arrive in order with real data, and confirming the garbage "host" is gone after the `parseHosts` fix. `go build`/`vet`/`golangci-lint run`/`go test ./...` and `npm run build`/`lint`/`test` (96 tests) all clean. Rebuilt and redeployed `frontend`, `scan-orchestrator`, `webrecon`. **Not yet done:** a visual check in an actual browser — no browser is available in this environment, so the animation itself (as opposed to the data feeding it, which is verified) has not been eyeballed. Commit `4a50ebb`.

### 2026-08-30 (a) — a url-sourced project's report generation was broken at every layer, found and fixed by actually driving it live, not by reading code

**Goal:** the user asked for a real, non-fabricated BOM report, end to end. Their one real registered project, `RNR Consulting` (`source_type=url`, registered via last session's URL flow), had never produced a scan or a report. Debugging this live, through the real running stack rather than by inspecting code in isolation, turned up seven independent, real bugs — each one masking the next, so the user kept hitting a new failure every time the previous one was fixed.

**The chain, in the order it was actually hit:**

1. `ProjectWizard.tsx`'s submit flow read a `createdId` **state** variable to attach the project's source immediately after `createProject` resolved, in the same synchronous continuation — a state setter never updates a value a closure already captured, only what the next render sees, so every `useSetPractices`/`useConnectRepo`/`useCreateWebSource`/`useUploadFile` call went out bound to `''`. RNR Consulting's row existed; its URL was silently never attached. **Already partly fixed, uncommitted, at session start** (the four hooks were reworked to take the project id as a mutate-call argument instead of a hook-constructor argument) — this session finished verifying and shipping it.
2. `GenerateFlow.tsx`'s `sourceKindFor()` had no case for `'url'` — so even a correctly-registered url project hit "has no scannable source" the instant Generate's Run button was clicked, for a source kind `services/webrecon` has fully supported since last session's Milestone 5. Added the missing case.
3. `libs/go-shared/authz/matrix.go` was missing `{ResourceReport, ActionCreate}` **entirely**, since the very commit that introduced `ResourceReport`'s other three permissions (`5463b9e feat(phase-6)`). The matrix fails closed on a missing entry (`Allow`'s own doc comment says so) — `POST /v1/reports` has 403'd for every role, every tenant, for this codebase's entire history. `TestAuthzMatrix` only iterates permissions already IN the matrix, so it structurally could never have caught an omission — that gap in the test itself is still open, flagged, not fixed this session. Added the entry (`RoleAnalyst` floor, matching `ResourceScan.ActionRun`/`ResourceCSAF.ActionCreate`'s reasoning) and the matching row to `docs/05-SECURITY-MODEL.md`'s RBAC table.
4. `services/webrecon`'s own `blob.Put` call built its storage key as `job.Output.Prefix + "/webrecon.json"` — and `Output.Prefix` defaults to `"s3://axebom/..."`, a URI, not a bucket-relative key. MinIO rejected it deep in the SDK with "Object name contains unsupported characters" (naming neither the key nor the colon), on a retryable path, so the job redelivered on JetStream's `[30s, 2m, 8m]` backoff and burned all `MaxDeliver=4` attempts before the scan could ever leave `queued`. This exact bug had already been found and fixed **once**, locally, in the fetcher's own upload code (`objectKeyPrefix` in `libs/go-shared/fetcher/archive.go`) — proof it belonged at the actual shared boundary. Added `blob.TrimKeyScheme`, applied inside `Store.Put`/`Get`/`PresignedGet` so every current and future caller is covered, not just the one that happened to need it first; added `blob_test.go` (the package had zero prior test coverage).
5. `frontend/src/lib/scans.ts`'s `isTerminal()` checked for `'succeeded'`/`'partial'` — neither of which `scan.scans`' own CHECK constraint or `events.ScanStatus` has ever produced. The real values are `'completed'`/`'completed_with_errors'`, so this returned "still running" for the two most common finished-scan outcomes, unconditionally. Used by `ProjectScans.tsx`'s "running" indicator, which never cleared for an actually-finished scan. Fixed the vocabulary; added `scans.test.ts` (also zero prior coverage).
6. `frontend/src/routes/scans/ScanProgress.tsx`: the progress WebSocket reconnected forever even after a scan finished — the server's deliberate clean close ("scan already finished", `websocket.StatusNormalClosure`) was indistinguishable, to `ws.ts`'s dumb transport-layer reconnect loop, from a real failure, because that loop is deliberately transport-only and does not parse scan status. Fixed at the one place that actually knows the scan is done: the page itself now disposes the connection from inside `onProgress` once it observes a terminal status (via the now-corrected `isTerminal`).
7. `services/report/internal/worker/worker.go`: a report is deliberately queued the instant its scan is created, before the scan has run — `bomsource.go`'s own error message already says "the scan may still be running." That race closes long before a slow git-sourced scan's report gets rendered. It does not for a fast webrecon scan: confirmed live, a report queued at `19:25:05.719` was marked **permanently** failed at `19:25:05.737` — 18ms later — while `normalize.bom_documents` for the same scan wasn't written until `19:25:07.622`, ~1.9s after. Classifying the error as retryable (`errs.InternalDependency`, which `queue.go`'s `retryable()` already retries) turned out **not to fix it**: `Render`'s own deferred marker calls `svc.Fail` — `status='failed'` — unconditionally on any error, before `queue.go` ever decides retryable-or-not, and `ClaimForRender` only claims from `status='queued'`; a JetStream redelivery's `Claim` found the row already `failed` and silently no-op'd as "already claimed." (The blob-store-failure path a few lines below has the identical, still-open gap — flagged, not touched, out of scope.) Fixed with a small bounded **in-process** retry (`loadNormalizedBOM`, 5 attempts / 3s apart, only on `store.ErrNotFound`) that sidesteps the broken claim/retry interaction entirely rather than trying to fix it generally. Added `worker_test.go` (zero prior coverage) — five cases including the exact reproduced race, give-up-after-N-attempts, non-`ErrNotFound` stays immediate/terminal, and context-cancellation.
8. Even after (1)–(7), the user kept seeing `WebSocket connection ... failed` on every new scan. Root cause, traced through the actual proxy chain and the vendored `coder/websocket@v1.8.15` source: a real browser always sends `Origin: https://192.168.30.202:5173`; `services/gateway`'s reverse proxy deliberately rewrites the outbound `Host` header to the upstream's own address for ordinary HTTP routing (`proxy.go`'s `Rewrite`, correct and untouched); `scan-orchestrator`'s `Progress` handler called `websocket.Accept` with `InsecureSkipVerify:false` and no `OriginPatterns`, and the library's own same-origin check only auto-authorizes when `Host == Origin`'s host — never true here, so it 403'd **every real browser handshake, unconditionally, before 101, regardless of scan speed**. curl never reproduced it because curl sends no `Origin` header at all, which is exactly what made every earlier manual curl-based verification this session look clean. (The scan-speed correlation genuinely looked causal in the moment — three real repros did happen to be fast scans — but the code path has no timing dependency at all.) Fixed by threading this service's existing `FRONTEND_URL` config value (already wired for notification links) through to `handler.New` as `OriginPatterns`. Added `progress_origin_test.go`: a real hijacked-connection handshake test proving the browser-shaped `Host`≠`Origin` mismatch now succeeds *and* a genuinely foreign origin is still rejected (this handler had zero prior test coverage).

**Also done as part of closing this out:**
- Pulled the still-missing `projectdiscovery/subfinder:v2.16.0` image (flagged as a gap in a prior session, never actually pulled) — verified live: a fresh scan went from `hosts:1` (root-page-only degrade) to `hosts:2` (real subdomain discovery running).

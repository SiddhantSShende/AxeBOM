...
**Current phase:** Phase 3 — ✅ **complete** (backend; frontend auth screens deferred — see below)
**Next action:** `Implement Phase 4 of EncoreBOM. Read docs/phases/PHASE-04-projects-sources.md.`
...
| Go services (8) | 🟡 **auth is real**; gateway has rate limiting; other 6 are scaffolds |
| **Auth service** | ✅ **register, login, refresh+reuse detection, logout, me, GitHub OAuth, invitations** |
| **RBAC** | ✅ 48 permissions, fails closed; route-guard test parses `routes.go` |

...
func runDB(ctx context.Context, args []string) error {
	if len(args) == 0 {
		return fmt.Errorf("usage: encorebom db <migrate|down|down-all|status|reset|seed|verify-rls>")
	}

	obs.InitLogger(obs.LogConfig{Service: "encorebom-db", Level: "info", Format: "text"})

...
// bridge (`encorebom sandbox run`). See CLAUDE.md §Conventions — anything a
...
func runSource(ctx context.Context, args []string) error {
	if len(args) == 0 {
		return errors.New("usage: encorebom source materialize [flags]")
	}
	switch args[0] {
	case "materialize":
...
const stampName = ".encorebom-source.json"
...
func sourceMaterialize(ctx context.Context, args []string) error {
	...
	fs.Usage = func() {
		fmt.Fprintln(os.Stderr,
			"Usage: encorebom source materialize --uri <key> --sha256 <digest> --dest <dir>")
		fmt.Fprintln(os.Stderr,

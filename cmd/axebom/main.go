// Command axebom is the AxeBOM operator CLI.
//
...
type command struct {
	name    string
	summary string
	// phase records which build phase introduces the command, so `axebom`
...
// `axebom verify` exits 3 when a signature does not verify and 1 when the
...
func init() {
	commands = []command{
		...
		{"iam", "Provision and inspect the ZITADEL identity tier", 3, runIAM},
...
func main() {
		...
		if c.name == name {
			if err := c.run(ctx, os.Args[2:]); err != nil {
				...
				if errors.As(err, &ee) {
					...
					if ee.err != nil {
						fmt.Fprintf(os.Stderr, "axebom %s: %v\n", name, ee.err)
					}
					os.Exit(ee.code)
				}
				fmt.Fprintf(os.Stderr, "axebom %s: %v\n", name, err)
	...
	fmt.Fprintf(os.Stderr, "axebom: unknown command %q\n\n", name)
	usage()
	os.Exit(2)
}

func usage() {
	fmt.Fprintf(os.Stderr, "AxeBOM operator CLI (%s)\n\n", Version)
	fmt.Fprintln(os.Stderr, "Usage: axebom <command> [flags]")

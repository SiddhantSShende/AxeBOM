...
import (
	...
	"github.com/axebom/axebom/libs/go-shared/compliance"
)
...
func runProfile(ctx context.Context, args []string) error {
	if len(args) == 0 {
		return fmt.Errorf("usage: axebom profile <lint|gen|show|guardrails|evidence> [flags]")
	}
	switch args[0] {
	case "lint":
...
func profileGen(args []string) error {
	...
	outPy := fs.String("out-py", "libs/py-shared/axebom_shared/model", "Python output directory")
